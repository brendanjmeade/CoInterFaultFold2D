...
% Plot displacements (fault + fold + block vs. scaled fault : the aliasing problem)
    UX_temp = uxfault + uxfold_ds + uxfold_ts + uxblock + uxdetach;
    UX_temp = uxfault + scale1.*uxfold_ds + 0.0.*uxfold_ts + 0.0.*uxblock + uxdetach;
    bjm_ux_co = UX_temp;
...
% Plot displacements (fault + fold + block vs. scaled fault : the aliasing problem)
    UX_temp = uzfault + scale1.*uzfold_ds + 0.0.*uzfold_ts + 0.0.*uzblock + uzdetach;
    bjm_uz_co = UX_temp;
    UZ_temp = (uzfault + uzdetach);
    scale = UZ_temp(:) \ UX_temp(:);
    UZ_temp = scale*UZ_temp;
...
% Plot interseismic velocities (long term)
    ...
    UX_temp = uxfault + scale1.*uxfold_ds + 0.*uxfold_ts + 0.*uxblock + uxdetach;
    UZ_temp = uzfault + scale1.*uzfold_ds + 0.*uzfold_ts + 0.*uzblock + uzdetach;
    %UX_temp = uxfault + uxfold_ds + uxfold_ts + uxblock + uxdetach;
    %UZ_temp = uzfault + uzfold_ds + uzfold_ts + uzblock + uzdetach;
    UX_total_all = UX_temp;
    UZ_total_all = UZ_temp;
...
% Save four fields to mat file
save('MakeMatU.mat', 'bjm_ux_inter', 'bjm_uz_inter', 'bjm_ux_co', 'bjm_uz_co');